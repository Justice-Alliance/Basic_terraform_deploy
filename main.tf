...
provider "aws" {
    region = var.region
    access_key = "AKIAQ2HXPMTCMDORX7AP"
    secret_key = "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"
}
...
# Implantation d'une Instance EC2 -ubuntu
...
resource "aws_security_group" "security-group-1" {
  name = "${random_pet.stack_name.id}-security-group-1"
  lifecycle {
    create_before_destroy = true
  }
}

resource "aws_security_group_rule" "security-group-1-inbound" {
  type = "ingress"
...
# Implantation d'une ELB pour une Instance EC2 -ubuntu
resource "aws_elb" "instance-devops" {
  ...
  instances                   = [aws_instance.ubuntu-ec2-instance-devops.id]
...
# Implantation d'une AUtoScaling Group pour une Instance EC2 -ubuntu et ELB
...
resource "aws_autoscaling_policy" "scale-up" {
    name = "${random_pet.stack_name.id}-sacle-up"
    scaling_adjustment = 1
    adjustment_type = "ChangeInCapacity"
    cooldown = 300
    autoscaling_group_name = "${aws_autoscaling_group.auto-scaling-group-01.name}"
}

resource "aws_autoscaling_policy" "scale-down" {
    name = "${random_pet.stack_name.id}-sacle-down"
    scaling_adjustment = -1
    adjustment_type = "ChangeInCapacity"
    cooldown = 300
    autoscaling_group_name = "${aws_autoscaling_group.auto-scaling-group-01.name}"
...
resource "aws_cloudwatch_metric_alarm" "memory-high" {
    alarm_name = "${random_pet.stack_name.id}-memory-high"
    ...
    dimensions = {
        AutoScalingGroupName = "${aws_autoscaling_group.auto-scaling-group-01.name}"
    }
}

resource "aws_cloudwatch_metric_alarm" "memory-low" {
    alarm_name = "${random_pet.stack_name.id}-memory-low"
